...
int HID_API_EXPORT_CALL hid_get_usage_page(hid_device *dev)
{
    int32_t res;
    res = get_int_property(dev->device_handle, CFSTR(kIOHIDDeviceUsagePageKey));
    if (!res)
        res = get_int_property(dev->device_handle, CFSTR(kIOHIDPrimaryUsagePageKey));
    return res;
}

//hid_device * HID_API_EXPORT hid_open(unsigned short vendor_id, unsigned short product_id, const wchar_t *serial_number)
hid_device * HID_API_EXPORT hid_open(unsigned short vendor_id, unsigned short product_id, unsigned short usage_page)
{
	/* This function is identical to the Linux version. Platform independent. */
